...
class Warehouse(gym.Env):
    ...
    def _execute_forward(self, agent: Agent) -> None:
        ...
        # New: Consume battery for movement
        agent.battery = max(0, agent.battery - _BATTERY_CONSUMPTION_MOVE)
    ...
    def _execute_load(self, agent: Agent, rewards: np.ndarray[Any, np.dtype[np.float64]]) -> np.ndarray[Any, np.dtype[np.float64]]:
        ...
        if shelf_id:
            if (
                ...
                agent.busy = False
                # New: Consume battery for loading
    ...
    def _execute_unload(self, agent: Agent, rewards: np.ndarray[Any, np.dtype[np.float64]]) -> np.ndarray[Any, np.dtype[np.float64]]:
        ...
        if not self._is_highway(agent.x, agent.y):
            if (
                ...
                # New: Consume battery for unloading
                agent.battery = max(0, agent.battery - _BATTERY_CONSUMPTION_LOAD)
    ...
    def _execute_charge(self, agent: Agent) -> None:
        # New: Charge if at or adjacent to a charging station
        for station in self.charging_stations:
            station_x, station_y = station.x, station.y
            if abs(agent.x - station_x) <= 0.1 and abs(agent.y - station_y) <= 0.1:
    ...
    def step(
        ...
        rewards -= 0.001
        # New: Apply penalty for low battery
        for i, agent in enumerate(self.agents):
            if agent.battery < 10:
                rewards[i] += _LOW_BATTERY_PENALTY
