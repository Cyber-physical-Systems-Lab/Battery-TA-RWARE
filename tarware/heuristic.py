...
class MissionType(Enum):
    ...
    RETURNING = 2
    DELIVERING = 3
    CHARGING = 4  # New: Mission type for recharging
...
def heuristic_episode(env, render=False, seed=None, save_gif=False, gif_path="episode.gif"):
    # non_goal_location_ids corresponds to the item ordering in `get_empty_shelf_information`
    non_goal_location_ids = np.array(env.shelf_action_ids)
    ...
    while not done:
        ...
        # New: Prioritize charging for agents with low battery
        for agent in agents:
            if agent.battery < BATTERY_LOW_THRESHOLD and agent not in assigned_agvs and agent not in assigned_pickers:
                # Find nearest available charging station
                # Check stations that are not occupied by agents currently there
                occupied_stations = {(a.x, a.y) for a in agents}
                # Also check stations that are already assigned to other agents for charging
                assigned_charging_stations = {(m.location_x, m.location_y) for m in assigned_agvs.values() if m.mission_type == MissionType.CHARGING}
                assigned_charging_stations.update({(m.location_x, m.location_y) for m in assigned_pickers.values() if m.mission_type == MissionType.CHARGING})
                available_stations = [s for s in env.charging_stations if (s.x, s.y) not in occupied_stations and (s.x, s.y) not in assigned_charging_stations]
                if available_stations:
                    charging_paths = [env.find_path((agent.y, agent.x), (s.y, s.x), agent, care_for_agents=False) for s in available_stations]
                    charging_distances = [len(p) if p else float('inf') for p in charging_paths]
                    if charging_distances:
                        min_idx = np.argmin(charging_distances)
                        closest_station = available_stations[min_idx]
                        station_id = coords_original_loc_map[(closest_station.y, closest_station.x)]
                        if agent.type == AgentType.AGV:
                            assigned_agvs[agent] = Mission(MissionType.CHARGING, station_id, closest_station.x, closest_station.y, timestep)
                            print(f"Assigned AGV {agent.id} to charge at ({closest_station.x}, {closest_station.y}), battery {agent.battery}")
                        elif agent.type == AgentType.PICKER:
                            assigned_pickers[agent] = Mission(MissionType.CHARGING, station_id, closest_station.x, closest_station.y, timestep)
                            print(f"Assigned PICKER {agent.id} to charge at ({closest_station.x}, {closest_station.y}), battery {agent.battery}")
        ...
        for picker in pickers:
            if picker in assigned_pickers and (picker.x == assigned_pickers[picker].location_x) and (picker.y == assigned_pickers[picker].location_y):
                if assigned_pickers[picker].mission_type != MissionType.CHARGING:
        ...
        for agv, mission in assigned_agvs.items():
            if mission.mission_type == MissionType.CHARGING:
                # If at or adjacent to charging station and battery low, stop moving and start charging
                station_x, station_y = mission.location_x, mission.location_y
                if (abs(agv.x - station_x) <= 0.1 and abs(agv.y - station_y) <= 0.1) and agv.battery < _BATTERY_FULL:
                ...
                # Check if battery is now full and can remove mission
                if agv.battery >= _BATTERY_FULL:
                    agvs_to_remove.append(agv)
            else:
                actions[agv] = mission.location_id if not agv.busy else 0
        
        # Remove completed missions (battery full or mission accomplished)
        for agv in agvs_to_remove:
            assigned_agvs.pop(agv)
        ...
        for picker, mission in assigned_pickers.items():
            if mission.mission_type == MissionType.CHARGING:
                # If at or adjacent to charging station and battery low, stop moving and start charging
                station_x, station_y = mission.location_x, mission.location_y
                if (abs(picker.x - station_x) <= 0.1 and abs(picker.y - station_y) <= 0.1) and picker.battery < _BATTERY_FULL:
        ...
        # Remove completed charging missions
